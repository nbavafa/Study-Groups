...
export function validateEmail(email) {
    ...
    if (filter.test(email)) {
      if (email.split("@")[1].split(".").pop() === "edu") {
        return true;
      }
    }
...
export function validate(form) {
    ...
    keys.slice(0, length).map(field => {
        if (field !== "error"){
            ...
            if (isEmpty(value)){
                if (field === "email") {
                  error[field] = 'Your university ' + field + ' is required';
                }
                else if (field === "confirm_password") {
                  error[field] = 'Your password is required';
                }
                else if (field === "username") {
                  error[field] = 'Your name is required'
                }
                else {
                ...
                if (type === "email" && !validateEmail(value)) {
                    error[field] = 'Enter a valid university email address';
                    success = false;
